...
def latex_wilcoxon_table(results,
                         desc,
                         problem):
    ...
    content = df.to_latex(index=False,
                          float_format="%.2g",
                          escape=False,
                          label="wilcoxon_test:{:s}".format(problem),
                          caption="Significance Test Using Wilcoxon Test for {:s}".format(desc))
...
for problem in data:
    ...
    results = data[problem]['wilcoxon_table']

    content = latex_wilcoxon_table(results,
                                   desc,
                                   problem)

    with open("latex/includes/{:s}_wilcoxon_test.tex".format(problem), "w") as f:
        f.write(content)
