...
short_problem_desc = {'spring_problem': 'Spring',
                      'pressure_vessel_problem': 'Vessel',
                      'pressure_vessel_problem_original': 'Vessel'}
...
def wilcoxon_test(sol0,
                  ...
                  sol5):
    """
    Executes Wilcoxno Similarity Test
    :param sol0: solutions for algorithm 0
    :param sol1: solutions for algorithm 1
    :param sol2: solutions for algorithm 2
    :param sol3: solutions for algorithm 3
    ...
    :return: results
    """
    solutions = [sol0, sol1, sol2, sol3, sol4, sol5]
    solvers = ['EP', 'ES', 'GA', 'BeesA', 'FFA', 'PSO']
    results = list()
    results.append(["---"] + solvers)
    n_results = np.zeros((6, 6))
    for i in range(0, 6):
        for j in range(0, 6):
            if i != j:
                res = stats.wilcoxon(solutions[i], solutions[j])
                n_results[i][j] = n_results[j][i] = res.pvalue

    for i in range(0, 6):
        results.append([solvers[i]] + n_results[i].tolist())
    return results
...
def ordinary_statistics(fit_results):
    ...
    for problem in fit_results:
        ...
        stats[problem]['wilcoxon_table'] = wilcoxon_test(solver_data['solve_ep'],
                                                         solver_data['solve_es'],
                                                         solver_data['solve_ga'],
                                                         solver_data['solve_beesa'],
                                                         solver_data['solve_ffa'],
